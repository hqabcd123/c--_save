...
#define UNDEUX "./Audio/undeux.wav"
#define OUTPATH "./Audio/outUndeux.wav"
/*
resources: https://stackoverflow.com/questions/13660777/c-reading-the-data-part-of-a-wav-file
http://knowledge-teaching.blogspot.com/2013/09/wav.html
main idea of this is cut header and data by part and just filp data along
but the problem is we don't know how long header szie is
this is a bug we need to solve*/
...
int main(){
    ...
    if (bytesRead > 0){
        ...
        for (int i = 0; i < 77; i++)//int i = 0; i < headerSize; i++, header end on 139
        {
            temp[i] = value[i];
        }
        for (int i = (length/2)-1, j = 77; i > 77; i--, j++)
        //int i = (length/2)-1, j = headerSize; i > headerSize; i--, j++
        {
            temp[j] = value[i];
        }
